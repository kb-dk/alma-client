...
public class AlmaClient extends AlmaRestClient {
     ...
     * @param limit Optional. Default: 25	Maximum number of results to return - between 25 and 1000 (multiples of 25)
     * @param col_names Optional. Default: true	Include column heading information. Default: true. To ensure consistent sort order it might be required to turn it off (col_names=false).
     * @return
     */
    public dk.kb.alma.client.analytics.Report startReport(String reportPath,
        ...
        return dk.kb.alma.client.analytics.Report.parseFromAlmaReport(get(constructLink()
     ...
     *
     * @param reportPath Full path to the report
     * @param token A resumption token generated by a /reports response.
     * @return
     */
    public dk.kb.alma.client.analytics.Report continueReport(String reportPath,
                                                             String token) {
    ...
    private Integer restrictLimit(@Nullable Integer limit) {
        final double a = Math.ceil(Math.min(1000, Math.max(0, limit == null ? 25 : limit) / 25.0));
        limit = (int) (a * 25);
        return limit;
    }
