...
public class AlmaClient extends AlmaRestClient {
    ...
    public dk.kb.alma.client.analytics.Report continueReport(dk.kb.alma.client.analytics.Report report)
        ...
        final WebClient query = constructLink()
                                        .path("/analytics/reports")
                                        .query("token", report.getToken());
        System.out.println(query.getCurrentURI());
        final Report almaReport = get(query,Report.class, false);
        return dk.kb.alma.client.analytics.Report.parseFromAlmaReport(almaReport, report);
    }
